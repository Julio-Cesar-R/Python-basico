...
@app.route("/productos/<string:product_name>")#muestra la informacion relacionada con el id enviado
def get_producto(product_name):
    product_found=[producto for producto in producto if producto["name"]==product_name]
...
@app.route("/productos/<string:product_name>",methods=["PUT"])#Actualiza la informacion de un producto mediante el id
...
def eliminar_producto(product_name):
    product_found = [productos for productos in producto if productos["name"] == product_name]
    if (len(product_found) > 0):
        producto.remove(product_found[0])
        return jsonify({"MESSAGE":"PRODUCTO ELIMINADO SATISFACTORIAMENTE","PRODUCTOS":producto})
    else:
